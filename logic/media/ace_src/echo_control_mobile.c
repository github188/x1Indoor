...
#ifdef AEC_DEBUG
    aecm->aecmCore->farFile = fopen(CFG_PUBLIC_DRIVE"/aecFar.pcm","wb");
	if(aecm->aecmCore->farFile==0)
		printf("WebRtcAecm_Create:Open farFile error!");
    aecm->aecmCore->nearFile = fopen(CFG_PUBLIC_DRIVE"/aecNear.pcm","wb");
	if(aecm->aecmCore->nearFile==0)
		printf("WebRtcAecm_Create:Open nearFile error!");
    aecm->aecmCore->outFile = fopen(CFG_PUBLIC_DRIVE"/aecOut.pcm","wb");
	if(aecm->aecmCore->outFile==0)
		printf("WebRtcAecm_Create:Open outFile error!");
    //aecm->aecmCore->outLpFile = fopen("aecOutLp.pcm","wb");

    aecm->bufFile = fopen(CFG_PUBLIC_DRIVE"/aecBuf.dat", "wb");
    aecm->delayFile = fopen(CFG_PUBLIC_DRIVE"/aecDelay.dat", "wb");
    aecm->preCompFile = fopen(CFG_PUBLIC_DRIVE"/preComp.pcm", "wb");
    aecm->postCompFile = fopen(CFG_PUBLIC_DRIVE"/postComp.pcm", "wb");
#endif // AEC_DEBUG
    return 0;
}
                                 ...
                                 WebRtc_Word16 nrOfSamples, WebRtc_Word16 msInSndCardBuf)
{
    ...
    aecm->ECstartup = 0; // Enable the AECM // by zxf
...
//////////
//by zxf
int WebRtcAecm_GetLastDelay(void* aecmInst)
{
    aecmob_t *aecm = aecmInst;

	if(aecm==NULL)
		return 0;
	if(aecm->aecmCore==NULL)
		return 0;
	if(aecm->aecmCore->delay_estimator==NULL)
		return 0;

	return WebRtc_last_delay(aecm->aecmCore->delay_estimator);
}
//////////
