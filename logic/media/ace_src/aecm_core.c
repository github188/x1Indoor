...
//#define WEBRTC_DETECT_ARM_NEON  1
...
//typedef unsigned __int64 uintptr_t;//by zxf
...
void printfdebuginfo(char * debugmeg);
...
//by zxf
#ifdef AEC_DEBUG
    (void)fwrite(farend, sizeof(int16_t), PART_LEN, aecm->farFile);
    (void)fwrite(nearendNoisy, sizeof(int16_t), PART_LEN, aecm->nearFile);
#endif
...
//by zxf
	...
	if(delay>2)
	{
  		sprintf(debugmeg,"WebRtc_DelayEstimatorProcessFix delay=%d", delay);
		printfdebuginfo(debugmeg);
	}
	*/
...
//by zxf
#ifdef AEC_DEBUG
    (void)fwrite(output, sizeof(int16_t), PART_LEN, aecm->outFile);
#endif
