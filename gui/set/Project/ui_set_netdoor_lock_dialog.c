/*************************************************
  ...
  File name:  	ui_set_netdoor_lock_dialog.c
  Author:     	caogw
  Version:    	2.0
  Date: 		2015-06-23
  Description:  
...
#define DEV_ICON_W				100
#define DEV_ICON_H				30
#define INPUT_CTRL_XPOS			140
#define INPUT_CTRL_YPOS			75
#define INPUT_CTRL_WPOS			DEV_ICON_W
#define INPUT_CTRL_HPOS			DEV_ICON_H
#endif
...
/************************函数定义************************/
CREATE_FORM(FrmNetdoorLockDialog);
...
static void draw_bk(HDC hdc)
{
	...
	TextRc.bottom = TextRc.top + FONT_20;
	DrawText(hdc, get_str(SID_Net_Door_LockTime), -1, &TextRc, DT_CALCRECT|DT_LEFT|DT_VCENTER|DT_SINGLELINE);
	DrawText(hdc, get_str(SID_Net_Door_LockTime), -1, &TextRc, DT_LEFT|DT_VCENTER|DT_SINGLELINE);
}
...
static void RightCtrlCommand(HWND hDlg, LPARAM lParam)
{
	...
	switch(image)
	{
	    case SID_Right_Sure:
			...
			text = atoi(tmp);
			g_locktime = text;
			SendMessage(g_ParenthDlg, WM_RELOAD, text, 0);
			close_page();
			break;
...
/*************************************************
  ...
  Others:		
*************************************************/
uint32 ui_netdoor_lock_dialog_win(HWND hDlg, uint32 textid, int32 num)
{
	g_PageTextID = textid;
	g_ParenthDlg = hDlg;
	g_locktime = num;
	form_show(&FrmNetdoorLockDialog);
